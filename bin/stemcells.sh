...
if ! $BOSH -n stemcells | grep -q "ubuntu-trusty.*$RABBITMQ_STEMCELL_VERSION"; then
  $BOSH upload-stemcell https://bosh.io/d/stemcells/bosh-${IAAS_INFO}-ubuntu-trusty-go_agent?v=${RABBITMQ_STEMCELL_VERSION}
fi